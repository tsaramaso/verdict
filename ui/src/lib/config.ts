...
export const UI = {
	...
	autoAdvancePhases: [GAME_PHASES.TURN_START, GAME_PHASES.ROUND_OVER],

	// Phases where only active player can act
	activePlayerPhases: [GAME_PHASES.DRAWING, GAME_PHASES.AWAITING_ACTION],
...
export function calculateKnownSum(
	...
	return hand
		.filter(
			(slot): slot is Required<CardSlot> =>
