...
def _validate_target(
    ...
    return state, target_owner, target_index
...
def log_player_response(state: GameState, player_id: str) -> None:
    """Called when player submits action in simultaneous phase."""
    if state.phase in SIMULTANEOUS_PHASES:
        state.phase_responses[player_id] = True


def check_collection_complete(state: GameState) -> bool:
...
def get_phase_elapsed_seconds(state: GameState) -> float:
    """Get seconds elapsed since phase started."""
    if not state.phase_started_at:
        return 0.0
    return (datetime.now(UTC) - state.phase_started_at).total_seconds()
